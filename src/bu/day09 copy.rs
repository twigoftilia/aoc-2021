...
fn puzzle_2(rows: &str) -> Result<i32, &'static str> {
    ...
    low_points.iter().for_each(|p| {
        ...
        println!("BASSIN LEN {}   {}   {:?}   ", p, bassin.len(), bassin);
    });
...
9899965678
*/
fn find_bass(hf: &HF, new_pos: &HashSet<usize>, bassin: &mut Vec<usize>) {
    ...
    for posp in new_pos {
        ...
        if min.unwrap() > c_depth {
            bassin.push(pos);
            println!(" XXXXXXX  ADD  POS={} to BASSIN  {:?}    ", pos, bassin);
        ...
        println!(
            "   XXX POS={} MIN ad {:?} next: {:?}",
            pos, min, &next_iters_pos
        );

        if !next_iters_pos.is_empty() {
            find_bass(hf, &next_iters_pos, bassin);
        }
    }
